...
def get_gmail_service(): #get service from credentials
...
def handle_message(data):
    ...
    if service:
        emails_df = get_emails(service)
        # Store DataFrame in some accessible place for internal processing; here just reassigning for simplicity
        data['emails_df'] = emails_df

    question = data.get('question', 'Default question')
    # If ask_gpt function is designed to handle DataFrames
    response = ask_gpt(question, data['emails_df'] if 'emails_df' in data else pd.DataFrame(), service)
